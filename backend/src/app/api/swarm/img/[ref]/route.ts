...
const BEE = process.env.BEE_URL || "http://localhost:3323";
...
export async function GET(req: Request) {
  // Derive [ref] from the URL path; avoids Next 15's strict context typing
  const url = new URL(req.url);
  const segments = url.pathname.split("/");
  const refHex = (segments[segments.length - 1] || "").toLowerCase();
