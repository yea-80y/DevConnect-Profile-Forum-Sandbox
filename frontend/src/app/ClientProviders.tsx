...
const BEE_URL = process.env.NEXT_PUBLIC_BEE_URL || "http://localhost:3323";
...
export default function ClientProviders({ children }: { children: ReactNode }) {
 ...
 useEffect(() => {
  ...
  // 2) Fetch the platform feed owner from server (for all users)
  let alive = true;
  (async () => {
    try {
  ...
  return () => { alive = false; };
    ...
    useEffect(() => {
          ...
          for (const delay of [0, 150, 400]) {
            try {
              ...
              const r = await fetch(apiUrl("/api/auth/me"), {
                credentials: "include", // ← send cookies for user + admin auth (cross-origin)
                cache: "no-store",      // ← avoid stale cached responses
                headers: {
                  ...(subject ? { "X-Subject-Address": subject } : {}), // ← subject for cross-origin
                },
              });
      ...
      // initial fetch on mount or when subject changes
      fetchMe();
